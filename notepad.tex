...
\begin{algorithm}
\caption{Adam Optimizer \textcolor{blue}{(Bias Correction Experimental Focus)}}
\label{alg:adam}
\begin{algorithmic}[1]
\Require $(\eta_t)_{t\ge_1} \subset \R_{\ge0}, \beta_1, \beta_2\in [0, 1),  \epsilon \in \R_{>0}$ \Comment{Standard hyperparameters}
\Require $\theta_0$ \Comment{Initialised model weights}
...
\While{$\theta_t$ not converged}
    ...
    \State $\theta_t \gets \theta_{t-1} - \eta_t \cfrac{m_t}{\sqrt{v_t} + \epsilon}$
\EndWhile
\end{algorithmic}
\end{algorithm}
Experiments 
...
\end{cases}
\] 
Indeed, excluding noise, this is an analogue of a hidden linear layer of a neural network
%Let $X\in \R^{N\times d}$ be our feature matrix, $W\in \R^{d\times D}$ our weight matrix and $Y\in\R^{N\times D}$ our target space. $Y$ is then modelled as a linear function of  $X$ with noise.  $\varepsilon \in \R^{N\times D}$ with $ \varepsilon _{i,j} \sim \mathcal{N}(0, \sigma_j^2)$ for $\sigma_1^2, \ldots, \sigma _D^2 \in \R_{>0}$
\\
