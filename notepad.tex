...
\[
 L(
\]
\subsubsection{Proof of Orthogonal Update step}
...
\begin{theorem}{tensor product stuff}
we have: 
\end{theorem} 
...
L(W) = \|Y - XW\|_F^2 = \|\text{vec(Y - XW)}\|_2^2 = \|\text{vec}(Y) - (\mathbbm{1}_D \otimes X) \text{vec} (W)\|_2^2  
\end{align*}
which is simply a quadratic function of the form $f(x) = \|y - Ax\|_2^2$ with positive-definite Hessian $2A^{T}A$ (where $\otimes$ denotes the Kronecker Product).  \\
While such a simple problem admits an analytic solution (indeed $W^* = (X^{T}X)^{-1}X^{T}Y$ provided $X^{T}X$ is invertible), we study the case where the number of datapoints is large   \\
\\
