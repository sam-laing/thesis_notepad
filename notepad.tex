...
\end{itemize}
\section{Why is the same $\varepsilon $ Used Everywhere?}
...
\section{Nesting the Moving averages}
...
In our experiments comparing adam with and without bias correction, we found that certain choices of the pair $\beta_1, \beta_2$ caused a much greater discrepency in performance particularly when no learning rate schedule is used. \\
\\
...
\\
In the case of pretraining language models, we restrict our attention to transformer based models. In particular we make use of the \href{https://github.com/Niccolo-Ajroldi/plainLM/tree/main}{following enhanced implementation} of nanoGPT (\cite{Karpathy2022}) which includes 
\\
...
\\
An idea which has regained popularity in recent years is to consider the individual structure of each layer of the matrix and leverage this structural information to obtain a better update direction bespoke to each layer. \\
The 
...
\\
We reiterate the explanation from \cite{bernstein2025deriving} with some additional pedagogy. 
The essential idea behind Muon updates 
...
The \href{https://github.com/KellerJordan/cifar10-airbench}{CIFAR-10 speedrun repository} is a competitive benchmarking project. The goal is to construct a 
